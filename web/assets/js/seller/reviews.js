...
async function loadProductInfo(id) {
    try {
        ...
        if (result.success) {
            ...
            const imgSrc = firstImage ? (firstImage.startsWith('http') ? firstImage : `../assets/images/products/${firstImage}`) : '../assets/images/no-image.png';
        ...
        if (result.success && result.data.length > 0) {
            ...
            container.innerHTML = `
                <div class="no-reviews" style="text-align:center; padding:50px;">
                    <img src="../assets/images/stars.png" style="width:100px; opacity:0.5; margin-bottom:15px;" onerror="this.style.display='none'">
...
function goBackToProduct() {
    ...
    if (productId) {
        // Navigate to dashboard and show product detail
        // Store product ID in sessionStorage for dashboard to pick up
        sessionStorage.setItem('viewProductId', productId);
        window.location.href = 'dashboard.php';
    } else {
        // Fallback to store view
        window.location.href = 'dashboard.php';
    }
}
