...
function loadMyOrders() {
    ...
    container.innerHTML = '<p style="text-align:center; padding:20px;">Memuat riwayat pesanan...</p>';

    fetch('../api/user/get-orders.php')
        .then(res => res.json())
        .then(result => {
            if (!result.success || !result.data || result.data.length === 0) {
                container.innerHTML = '<p style="text-align:center; padding:20px;">Belum ada pesanan.</p>';
                return;
            }
            ...
            container.innerHTML = result.data.map(order => {
                ...
                if (order.status === 'pending') {
                    ...
                    actionBtn = `<button class="btn-done" onclick="finishOrder(${order.id})" style="background:#27ae60; color:white; border:none; padding:5px 10px; border-radius:4px; cursor:pointer;">Pesanan Diterima</button>`;
                ...
                const otherItemsText = itemCount > 1 ? `+ ${itemCount - 1} barang lainnya` : '';
                ...
                <div class="cart-item order-card" style="border:1px solid #eee; padding:15px; border-radius:8px; display:flex; gap:15px; align-items:center;">
                    ...
                    <div class="item-details" style="flex:1;">
                        <h4 style="margin:0;">${firstItem.name} <small style="color:#666; font-weight:normal;">${otherItemsText}</small></h4>
                        <p style="margin:5px 0; color:#888;">Toko: ${order.store_name || 'Seller'}</p>
                        <p class="item-price" style="font-weight:bold;">Total: Rp ${Number(order.total_amount).toLocaleString('id-ID')}</p>
                        <p class="item-variant" style="font-size:0.9rem;">Status: <span style="color:${statusColor}; font-weight:bold;">${statusText}</span></p>
                    </div>
                    <div class="item-actions">
                        ${actionBtn}
                        <button class="btn-detail" onclick="alert('Detail pesanan ID: ${order.id}')" style="margin-left:5px; padding:5px 10px;">Rincian</button>
                    </div>
                </div>
                `;
            }).join('');
        ...
        .catch(err => {
            console.error(err);
            container.innerHTML = '<p style="text-align:center; color:red;">Gagal memuat pesanan.</p>';
        });
}

window.finishOrder = function(orderId) {
    if (!confirm('Apakah barang sudah diterima dengan baik? Barang yang sudah diterima tidak dapat dikembalikan.')) return;
